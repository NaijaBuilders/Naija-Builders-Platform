...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $role = $this->currentUserRole($currentUserId);

        if ($role !== 'supplier') {
            return response()->json(['message' => 'Supplier dashboard only.'], 403);
        }

        return response()->json([
            'current_user' => $this->currentUserPayload($currentUserId),
            'metrics' => $this->buildSupplierDashboardData($currentUserId),
        ]);
    }
    ...
    public function analysis(Request $request)
    {
        ...
        if ($role !== 'supplier') {
            return response()->json(['message' => 'Supplier analytics only.'], 403);
        }
    ...
    public function buyer(Request $request)
    {
        ...
        return response()->json([
            ...
            'selected_saved_category_id' => $selectedSavedCategoryId,
        ]);
    }
    ...
    private function currentUserPayload(int $userId): array
    {
        $user = DB::table('users')
            ->select(['id', 'full_name', 'email', 'role', 'profile_image_path'])
            ->where('id', $userId)
            ->first();
    ...
    {
        return (string) DB::table('users')->where('id', $userId)->value('role');
    }
}
